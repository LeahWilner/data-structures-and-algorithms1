...
const count = (target, input) => {
  // Solution code here...
  return arrOfArr.reduce((acc, curr) => acc + curr.filter(val => val === num).length, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return arrOfArr.reduce((acc, curr) => acc + curr.reduce((a, b) => a + b, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return arrOfArr.map(subArr => subArr
...
let findShortest = (data) => {
    ...
    return data.reduce((shortest, char) => {
      if (!shortest || char.height < shortest.height) {
        return char;
      }
      return shortest;
    }, null).name;
